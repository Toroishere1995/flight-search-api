...
/**
 * Class that sort according to fare of flight with cheapest at the top.
 * 
 * @author ayushsaxena
 *
 */
public class SortCheapestService implements ISortService {

	/**
	 * Method implemented to sort with respect to fare of flight.
	 */
	@Override
	public void sortSearchedFlights(List<Object> flights) {
		...
		if (flights.get(0) instanceof FlightDirectResult) {
			...
			Collections.sort(convertedList, Comparator.comparingDouble(FlightDirectResult::getFareOfFlight));
			...
			Collections.sort(convertedList, Comparator.comparingDouble(FlightConnectingResult::getTotalFare));
		}
	}
